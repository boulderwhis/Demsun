...
if (housecommittees.hasOwnProperty(x)) {
  window.localStorage.setItem('housecommittee', housecommittees[x]);
  console.log("house committee identified : "+housecommittees[x]);
  $.ajax({
...
if (housesub.hasOwnProperty(g)) {
  console.log("house subcommittee id = "+housesub[g]);
  var y = housesub[g].substring(0,4);
  console.log(y);
  $.ajax({
    url: "https://api.propublica.org/congress/v1/116/house/committees/"+y+"/subcommittees/"+housesub[g]+".json" ,
    type: "GET",
    datatype: "json",
    headers: {'X-API-Key': 'vNgtOv93Sq3FsN6A9xTzD2Ffca8jtk8v7JSneL71'},
  }).done(function (data){
    ...
    if (window.localStorage.getItem('S1ID')==='fail') {
          ...
          '<div class="col-lg-12 text-center">' +
            ...
            '</div>'
                    ...
                    var u = {
                    rhetoric: `Sorry folks. It looks like we've had some trouble on our end. Try heading back to the Home page and selecting your issue again
                               or email admin@tds.com and we'll look into it as soon as possible. Thanks again for visiting`,
                    sarcasm: "Also, we know what the error was now. It was your fault. Nice job. You've ruined it for everyone"
                    };
                    document.getElementById("content").innerHTML=u.rhetoric;
                    $('#content').fadeIn(2000);
...
$.ajax({
  ...
  $.getJSON( "https://www.opensecrets.org/api/?method=candSummary&cid="+localStorage.getItem("crp_id")+"&cycle=2020&output=json&apikey=dcf447b80edb32e28cb162c2d0548e77").done( function ( opensecrets ) {
      ...
      '</div>'+
        '<div style="padding-top: 25px" class="col-sm-6">'+
          '<h1>Summary Profile</h1>'+
            '<p>major donors to the right. explore opensecrets api to create more diverse endpoints</p>'+
   //     '<h6>'+opensecrets.response.summary.@attributes+'</h6>'+
        '</div>'+
    '</div>'
)
    console.log(opensecrets)
